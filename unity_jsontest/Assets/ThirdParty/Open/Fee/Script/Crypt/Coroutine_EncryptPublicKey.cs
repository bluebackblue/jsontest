	...
	public class Coroutine_EncryptPublicKey : Fee.Crypt.OnCryptTask_CallBackInterface
	{
		...
		*/
		public System.Collections.IEnumerator CoroutineMain(Fee.Crypt.OnCryptCoroutine_CallBackInterface a_callback_interface,byte[] a_binary,string a_key)
		{
			...
			//result
			Task_EncryptPublicKey.ResultType t_result;

			//タスク起動。
			using(Fee.TaskW.Task<Task_EncryptPublicKey.ResultType> t_task = Task_EncryptPublicKey.Run(this,a_binary,a_key,t_cancel_token)){
			...
			if(t_result.errorstring != null){
				...
				this.result.errorstring = "Coroutine_EncryptPublicKey : null";
				yield break;
			}
		}
	}
