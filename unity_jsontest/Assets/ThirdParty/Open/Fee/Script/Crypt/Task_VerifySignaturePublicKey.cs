	...
	*/
	public class Task_VerifySignaturePublicKey
	{
		...
		*/
		...
		private static ResultType TaskMain(Fee.Crypt.OnCryptTask_CallBackInterface a_callback_interface,byte[] a_binary,byte[] a_signature_binary,string a_key,Fee.TaskW.CancelToken a_cancel)
		#else
		...
		/** 実行。
		*/
		public static Fee.TaskW.Task<ResultType> Run(Fee.Crypt.OnCryptTask_CallBackInterface a_callback_interface,byte[] a_binary,byte[] a_signature_binary,string a_key,Fee.TaskW.CancelToken a_cancel)
		{
			return new Fee.TaskW.Task<ResultType>(() => {
				return Task_VerifySignaturePublicKey.TaskMain(a_callback_interface,a_binary,a_signature_binary,a_key,a_cancel);
			});
		}
	}
}
