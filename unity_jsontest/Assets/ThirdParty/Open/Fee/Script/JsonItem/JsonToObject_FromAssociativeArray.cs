...
{
	/** JsonToObject_FromAssociativeArray
	*/
	public class JsonToObject_FromAssociativeArray
	{
		/** Convert
		*/
				...
				if(t_to_dictionary != null){
					...
					if(t_key_type == typeof(string)){
						...
						foreach(string t_listitem_key_string in t_keylist){
							...
							a_workpool.AddFirst(new JsonToObject_WorkPool_Item(JsonToObject_WorkPool_Item.ModeAddDictionary.Start,t_listitem_jsonitem,t_to_dictionary,t_listitem_key_string,t_listitem_valuetype));
						}
			...
			//class,struct
			{
				...
				foreach(System.Reflection.FieldInfo t_fieldinfo in t_fieldinfo_list){
					if(a_from_jsonitem.IsExistItem(t_fieldinfo.Name) == true){
						...
						a_workpool.AddFirst(new JsonToObject_WorkPool_Item(JsonToObject_WorkPool_Item.ModeFieldInfo.Start,t_jsonitem_classmember,t_fieldinfo,a_to_object));
					}
				}
